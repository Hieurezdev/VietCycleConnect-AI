...
class VectorStoreService:
    ...
    async def add_chunks_batch(
        ...
        try:
            ...
            for chunk_data, embedding in zip(chunks_data, embeddings, strict=False):
                headers_str = (
                    ",".join(chunk_data.get("headers", [])) if chunk_data.get("headers") else ""
                )
    ...
    async def search_similar(
        ...
        try:
            ...
            similarity_expr = text("1 - (embedding <=> CAST(:query_embedding AS vector))")

            stmt = select(KnowledgeChunk, similarity_expr.label("similarity")).where(
            ...
            results: list[tuple[KnowledgeChunk, float]] = [(row[0], float(row[1])) for row in rows]
    ...
    async def update_chunk(
        ...
        try:
            ...
            if content is not None:
                ...
                chunk.embedding = await self._embedding_service.embed_document_async(content)
    ...
    async def delete_chunk(self, session: AsyncSession, chunk_uuid: uuid.UUID) -> bool:
    ...
    async def count_chunks(self, session: AsyncSession, chunk_type: str | None = None) -> int:
